...
@section('content')
  <div class="col-md-8">
    ...
    action="{{route('users.update', ['id'=>$user->id])}}" method="POST">
      ...
      name="roles[]" class="form-control {{$errors->first('roles') ? "is-invalid" : "" }}"
      ...
      {{$errors->first('phone') ? "is-invalid" : ""}}" value="{{old('phone') ? old('phone')
      : $user->phone}}">
      ...
      {{$errors->first('address') ? "is-invalid" : ""}}">{{old('address') ? old('address')
